...
export interface MathematicsOptions {
  ...
  // biome-ignore lint/suspicious/noExplicitAny: HTML attributes
...
export const Mathematics = Node.create<MathematicsOptions>({
    ...
    return [
      {
        ...
        getAttrs: (dom) => ({
          latex: (dom as HTMLElement).getAttribute("data-latex") || (dom as HTMLElement).textContent || "",
        }),
      },
    ];
  },
  ...
  addNodeView() {
    return ({ node, HTMLAttributes, getPos, editor }) => {
      ...
      const clickHandler = (_evt: MouseEvent) => {
